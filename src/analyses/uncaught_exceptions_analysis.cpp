...
Function: uncaught_exceptions_domaint::get_subtypes
...
void uncaught_exceptions_domaint::get_subtypes(
  const irep_idt &type,
    ...
    forall_symbols(it, symbol_table.symbols)
    {
      ...
      if(it->second.is_type)
      {
        ...
        if(find(subtypes.begin(),
          ...
          continue;
        // get its base class
        irept::subt bases=to_class_type((it->second).type).bases();
        if(bases.size()>0)
        {
          irept base = bases[0];
...
Function: uncaught_exceptions_domaint::get_static_type
...
irep_idt uncaught_exceptions_domaint::get_static_type(const exprt &expr)
{
  if(expr.id()==ID_typecast)
  {
    return get_static_type(expr.op0().type());
  }
  else if(expr.id()==ID_symbol)
  {
    return get_static_type(expr.type());
  }
  return ID_empty;
}
  ...
  const namespacet &ns)
{
  ...
  case THROW:
  {
    exprt exc_symbol=instruction.code;

    // retrieve the static type of the thrown exception
    irep_idt type_id=get_static_type(exc_symbol.type());
    join(type_id);
  ...
  case CATCH:
  {
    if(!instruction.code.has_operands())
    {
      ...
      else // pop
      {
        ...
        {
          std::set<irep_idt> &caught=stack_caught.back();
          join(caught);
          // remove the caught exceptions
          for(auto exc_id : caught)
          {
            std::set<irep_idt>::iterator it=
...
void uncaught_exceptions_domaint::get_elements(std::set<irep_idt> &elements)
{
  elements=thrown;
}
